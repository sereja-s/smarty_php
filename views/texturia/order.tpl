...
<form id="frmOrder" action="/cart/saveorder/" method="POST">
	<table>
		...
		{foreach $rsProducts as $item name=products}
			<tr>
				{* порядквый номер продукта в корзине *}
				<td>{$smarty.foreach.products.iteration}</td>
				{* название продукта (товара) будет ссылкой *}
				<td><a href="/product/{$item['id']}/">{$item['name']}</a></td>
				{* кол-во покупаемого товара *}
				<td>
				...
				{* итоговая стоимость за данный товар (с учётом количества) *}
				<td>
	...
	{/if}
	{* если покупатель не зарегистрирован и не авторизован, то кнопка не появится *}
